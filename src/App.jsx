...
const apiBaseUrl = "https://gestao-obras-api.onrender.com";
    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          <li>✓ Controle completo de Obra</li>
          <li>✓ Controle completo de Etapa</li>
          <li>✓ Controle completo de Profissional</li>
          <li>✓ Controle completo de Alocacao Profissional</li>
        </ul>
      </div>
...
function PainelObra({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "endereco", "tipo", "cliente", "orcamento", "dataInicio", "dataFimPrevista", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const carregar = () => fetch(apiBaseUrl + "/api/obras", { headers: authHeaders(token) })
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/obras/" + editId : apiBaseUrl + "/api/obras";
    fetch(url, {
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "endereco", "tipo", "cliente", "orcamento", "dataInicio", "dataFimPrevista", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/obras/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["nome"] ?? "Obra")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/obras/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Obra")}</div>
            ...
            <div className="item-field"><b>cliente</b><span>{item["cliente"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>dataFimPrevista</b><span>{item["dataFimPrevista"] ? new Date(item["dataFimPrevista"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
      ...
      <label className="field-label">dataFimPrevista</label>
      <input type="datetime-local" value={form["dataFimPrevista"] ?? ""} onChange={e => setForm({...form, dataFimPrevista: e.target.value})} />
...
function PainelEtapa({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [pagina, setPagina] = React.useState(1);
  const [modoKanban, setModoKanban] = React.useState(false);
  const [obraList, setObraList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["obraId", "nome", "descricao", "ordem", "percentualConcluido", "dataPrevista", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/etapas", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/obras", { headers: authHeaders(token) }).then(r => r.json()).then(setObraList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/etapas/" + editId : apiBaseUrl + "/api/etapas";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["obraId", "nome", "descricao", "ordem", "percentualConcluido", "dataPrevista", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["obra"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/etapas/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
        ...
        <div className="kanban-board">
          {[...new Set(itensFiltrados.map(i => String(i.status ?? "Sem status")))].map(coluna => (
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["nome"] ?? "Etapa")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/etapas/" + item.id, {
                      method: "PUT", headers: { "Content-Type": "application/json", ...authHeaders(token) },
                      body: JSON.stringify({ ...item, status: e.target.value }),
                    }).then(carregar);
                  }}>
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Etapa")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>obraId</b><span>{item["obraId"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>percentualConcluido</b><span>{item["percentualConcluido"] != null ? Number(item["percentualConcluido"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            ...
            <div className="item-field"><b>obra</b><span>{item.obraId ? ((obraList.find(o => o.id === item.obraId) || {}).nome ?? ("#" + item.obraId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Etapa</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">obraId</label>
      <input type="number" value={form["obraId"] ?? ""} onChange={e => setForm({...form, obraId: e.target.value})} />
      <label className="field-label">nome</label>
      <input type="text" value={form["nome"] ?? ""} onChange={e => setForm({...form, nome: e.target.value})} />
      ...
      <label className="field-label">percentualConcluido</label>
      <input type="number" step="0.01" value={form["percentualConcluido"] ?? ""} onChange={e => setForm({...form, percentualConcluido: e.target.value})} />
      ...
      <select value={form["obra"] ?? ""} onChange={e => setForm({...form, obra: e.target.value})}>
        ...
        {(obraList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelProfissional({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "oficio", "telefone", "email", "disponivel", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/profissionals", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/profissionals/" + editId : apiBaseUrl + "/api/profissionals";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "oficio", "telefone", "email", "disponivel"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/profissionals/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Profissional")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>cpf</b><span>{item["cpf"] ?? "—"}</span></div>
            <div className="item-field"><b>oficio</b><span>{item["oficio"] ?? "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Profissional</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">nome</label>
      <input type="text" value={form["nome"] ?? ""} onChange={e => setForm({...form, nome: e.target.value})} />
      ...
      <label className="field-label">oficio</label>
      <input type="text" value={form["oficio"] ?? ""} onChange={e => setForm({...form, oficio: e.target.value})} />
...
function PainelAlocacaoProfissional({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [pagina, setPagina] = React.useState(1);
  const [modoKanban, setModoKanban] = React.useState(false);
  const [profissionalList, setProfissionalList] = React.useState([]);
  const [obraList, setObraList] = React.useState([]);
  const [etapaList, setEtapaList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["obraId", "profissionalId", "etapaId", "dataInicio", "dataFim", "diaria", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/alocacaoprofissionals", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/profissionals", { headers: authHeaders(token) }).then(r => r.json()).then(setProfissionalList).catch(() => {});
    fetch(apiBaseUrl + "/api/obras", { headers: authHeaders(token) }).then(r => r.json()).then(setObraList).catch(() => {});
    fetch(apiBaseUrl + "/api/etapas", { headers: authHeaders(token) }).then(r => r.json()).then(setEtapaList).catch(() => {});
  }, [token]);
  ...
  const montarCorpo = () => {
    const corpo = { ...form };
    ["obra", "profissional", "etapa"].forEach(k => {
      corpo[k + "Id"] = corpo[k] ? Number(corpo[k]) : null;
      delete corpo[k];
    });
    // Bug real (2026-06-26): <input type="datetime-local"> envia "YYYY-MM-DDTHH:mm"
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/alocacaoprofissionals/" + editId : apiBaseUrl + "/api/alocacaoprofissionals";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["obraId", "profissionalId", "etapaId", "dataInicio", "dataFim", "diaria", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["obra", "profissional", "etapa"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/alocacaoprofissionals/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
        ...
        <div className="kanban-board">
          {[...new Set(itensFiltrados.map(i => String(i.status ?? "Sem status")))].map(coluna => (
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["status"] ?? "AlocacaoProfissional")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/alocacaoprofissionals/" + item.id, {
                      method: "PUT", headers: { "Content-Type": "application/json", ...authHeaders(token) },
                      body: JSON.stringify({ ...item, status: e.target.value }),
                    }).then(carregar);
                  }}>
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["status"] ?? "AlocacaoProfissional")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>obraId</b><span>{item["obraId"] ?? "—"}</span></div>
            <div className="item-field"><b>profissionalId</b><span>{item["profissionalId"] ?? "—"}</span></div>
            <div className="item-field"><b>etapaId</b><span>{item["etapaId"] ?? "—"}</span></div>
            <div className="item-field"><b>dataInicio</b><span>{item["dataInicio"] ? new Date(item["dataInicio"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            ...
            <div className="item-field"><b>diaria</b><span>{item["diaria"] != null ? Number(item["diaria"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>obra</b><span>{item.obraId ? ((obraList.find(o => o.id === item.obraId) || {}).nome ?? ("#" + item.obraId)) : "—"}</span></div>
            <div className="item-field"><b>profissional</b><span>{item.profissionalId ? ((profissionalList.find(o => o.id === item.profissionalId) || {}).nome ?? ("#" + item.profissionalId)) : "—"}</span></div>
            <div className="item-field"><b>etapa</b><span>{item.etapaId ? ((etapaList.find(o => o.id === item.etapaId) || {}).nome ?? ("#" + item.etapaId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} AlocacaoProfissional</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">obraId</label>
      <input type="number" value={form["obraId"] ?? ""} onChange={e => setForm({...form, obraId: e.target.value})} />
      <label className="field-label">profissionalId</label>
      <input type="number" value={form["profissionalId"] ?? ""} onChange={e => setForm({...form, profissionalId: e.target.value})} />
      <label className="field-label">etapaId</label>
      <input type="number" value={form["etapaId"] ?? ""} onChange={e => setForm({...form, etapaId: e.target.value})} />
      <label className="field-label">dataInicio</label>
      <input type="datetime-local" value={form["dataInicio"] ?? ""} onChange={e => setForm({...form, dataInicio: e.target.value})} />
      ...
      <label className="field-label">diaria</label>
      <input type="number" step="0.01" value={form["diaria"] ?? ""} onChange={e => setForm({...form, diaria: e.target.value})} />
      <label className="field-label">status</label>
      <select value={form["status"] ?? "ATIVO"} onChange={e => setForm({...form, status: e.target.value})}>
        <option value="ATIVO">ATIVO</option>
        <option value="PENDENTE">PENDENTE</option>
      ...
      <select value={form["profissional"] ?? ""} onChange={e => setForm({...form, profissional: e.target.value})}>
        <option value="">Selecione...</option>
        {(profissionalList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
      <label className="field-label">etapa</label>
      <select value={form["etapa"] ?? ""} onChange={e => setForm({...form, etapa: e.target.value})}>
        <option value="">Selecione...</option>
        {(etapaList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("Obra");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Gestao Obras<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Obra" ? " active" : "")} onClick={() => setAba("Obra")}><span className="nav-ico">🗂️</span>Obra</button>
        <button className={"nav-btn" + (aba === "Etapa" ? " active" : "")} onClick={() => setAba("Etapa")}><span className="nav-ico">🗂️</span>Etapa</button>
        <button className={"nav-btn" + (aba === "Profissional" ? " active" : "")} onClick={() => setAba("Profissional")}><span className="nav-ico">🧑‍🔧</span>Profissional</button>
        <button className={"nav-btn" + (aba === "AlocacaoProfissional" ? " active" : "")} onClick={() => setAba("AlocacaoProfissional")}><span className="nav-ico">🧑‍🔧</span>AlocacaoProfissional</button>
      ...
      <main className="main">
        ...
        {aba === "Obra" && <PainelObra token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Etapa" && <PainelEtapa token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Profissional" && <PainelProfissional token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "AlocacaoProfissional" && <PainelAlocacaoProfissional token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
